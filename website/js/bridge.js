/**
 * bridge.js — FB Manager Extension Communication Bridge
 *
...
const Bridge = (function () {
  ...
  function onExtensionMessage(e) {
    ...
    // The extension's content script wraps the bg response in detail.data
    ...
    if (msgId && pendingResolves.has(msgId)) {
      const { resolve, reject } = pendingResolves.get(msgId);
      pendingResolves.delete(msgId);
      if (data.error) reject(new Error(data.error));
      else resolve(data);
    }

    _trigger(data.event || payload.event || 'message', data);
  }
