...
const API = (function () {
  ...
  /* ---------- Publishing ---------- */
  async function publishVideoByUrl(pageId, fileUrl, description = '', title = '', scheduledTime = null, pageAccessToken = '') {
    ...
    if (scheduledTime && scheduledTime > 0) {
      // Facebook requires published=false whenever scheduled_publish_time is set,
      // otherwise it errors with (#100) "You cannot specify a scheduled publish time on a published post."
      // scheduledTime is in Unix SECONDS (matches the rest of the codebase:
      // videoPost.js uses Math.floor(Date.now()/1000) and Math.floor(d.getTime()/1000)).
      const nowSec = Math.floor(Date.now() / 1000);
      const minOffsetSec = 10 * 60;                       // 10 minutes
      const maxOffsetSec = 6 * 30 * 24 * 60 * 60;         // ~6 months
      if (scheduledTime < nowSec + minOffsetSec) {
        throw new Error('Scheduled time must be at least 10 minutes in the future.');
      }
      if (scheduledTime > nowSec + maxOffsetSec) {
        throw new Error('Scheduled time cannot be more than 6 months in the future.');
      }
      params.published = 'false';
      params.scheduled_publish_time = scheduledTime;
    }
    return post(`/${pageId}/videos`, params, token);
  }
